...
export async function runAutoPr(
  ...
  const secretFindings = eligibleFindings.filter(
    (f) => f.rule === 'no-secrets/no-secrets' || f.rule === 'no-eval',
  );
  ...
  await createAutoPr(
    'auditguard/auto-fix-' + today,
    `fix(security): automated remediation for ${eligibleFindings.length} findings`,
    body,
    token,
  );
}

function applyNpmAuditFixes(): boolean {
  try {
...
function applySecretFixes(findings: Finding[]): boolean {
  let changed = false;

  for (const finding of findings) {
    if (finding.file === 'package.json') continue;

    core.warning(
      `Secret found in ${finding.file}:${finding.line} — manual review recommended. ` +
...
function generatePrBody(findings: Finding[]): string {
