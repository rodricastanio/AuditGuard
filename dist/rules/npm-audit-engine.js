...
function execNpmAuditJson(scanPath) {
    return new Promise((resolve, reject) => {
        execFile('npm', ['audit', '--json'], { cwd: scanPath, timeout: 60_000, maxBuffer: 10 * 1024 * 1024 }, (error, stdout) => {
            if (stdout) {
                resolve(stdout);
            }
            else if (error) {
                reject(new Error(`npm audit failed: ${error.message}${error.code ? ` (exit code ${error.code})` : ''}`));
            }
            else {
                resolve(stdout || '{}');
            }
